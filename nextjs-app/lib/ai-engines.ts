 ...
 * Portado do Omnisfera — sem tracking de workspace.
 */
...
function getApiKey(engine: EngineId, overrideKey?: string): string {
    ...
    if (k) {
        if (engine === "orange") return validateOpenAIKey(k, "apiKey override");
        return k;
    }
    switch (engine) {
        case "red": return getEnv("DEEPSEEK_API_KEY");
        case "blue": return getEnv("OPENROUTER_API_KEY") || getEnv("KIMI_API_KEY");
        case "green": return getEnv("ANTHROPIC_API_KEY");
        case "yellow": return getEnv("GEMINI_API_KEY");
        case "orange": return validateOpenAIKey(getEnv("OPENAI_API_KEY"));
        default: return validateOpenAIKey(getEnv("OPENAI_API_KEY"));
    }
}
...
export function getEngineError(engine: EngineId): string | null {
    ...
    if (key) return null;
    return `Configure a chave para ${ENGINE_NAMES[engine]}. Ver .env.local.`;
}

/** Chat completion para qualquer engine de texto. */
...
export async function visionAdapt(
    ...
    if (!v) throw new Error("Configure GEMINI_API_KEY ou OPENAI_API_KEY para visão.");

    if (v.engine === "yellow") {
        const { GoogleGenerativeAI } = await import("@google/generative-ai");
        const genAI = new GoogleGenerativeAI(v.key);
        const model = genAI.getGenerativeModel({ model: "gemini-2.0-flash" });
        const imagePart = { inlineData: { data: imageBase64, mimeType: mime || "image/jpeg" } };
