...
/**
 * Componente para exibir textos gerados com formatação rica
 * Suporte a markdown básico, imagens, listas e headings
 */
export function FormattedTextDisplay({ texto, titulo, className = "", mapaImagens }: Props) {
    ...
    const processarTexto = (texto: string): React.ReactNode[] => {
        ...
        const formatarLinhaComImagens = (linha: string): React.ReactNode => {
            ...
            if (matches.length === 0) return formatarLinha(linha);
            ...
            for (const match of matches) {
                ...
                if (match.index !== undefined && match.index > lastIndex) {
                    ...
                    if (textoAntes.trim()) partes.push(formatarLinha(textoAntes));
                }
            ...
            if (lastIndex < linha.length) {
                ...
                if (textoResto.trim()) partes.push(formatarLinha(textoResto));
            }
        ...
        linhas.forEach((linha, idx) => {
                ...
                if (imgBase64) {
                    elementos.push(
                        <div key={`img-block-${idx}`} className="my-4 flex justify-center">
                            <img src={`data:image/png;base64,${imgBase64}`} alt={`Imagem ${num}`}
                                className="max-w-full h-auto rounded-lg shadow-md border border-slate-200" style={{ maxHeight: "400px" }} />
                        </div>
                    );
                } else {
                    elementos.push(
            ...
            if (/^[-*•]\s+/.test(linhaTrim) || /^\d+\.\s+/.test(linhaTrim)) {
                if (!emLista) { finalizarLista(); emLista = true; }
                listaAtual.push(linhaTrim.replace(/^[-*•]\s+/, "").replace(/^\d+\.\s+/, ""));
                return;
            }
