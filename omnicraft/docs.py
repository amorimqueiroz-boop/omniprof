# docs.py — Geração de DOCX/PDF (Omnicraft independente)
"""
...
def construir_docx_final(
    ...
    """Constrói documento DOCX final. aluno vazio ou {"nome":"Turma"}."""
    aluno = aluno or {}
    nome_estudante = aluno.get("nome", "Turma")
    doc = Document()
    ...
    if not sem_cabecalho:
        titulo = doc.add_heading(f"{tipo_atv.upper()} - {materia.upper()}", 0)
        titulo.alignment = WD_ALIGN_PARAGRAPH.CENTER
        if titulo.runs:
            titulo.runs[0].font.size = Pt(16)
            titulo.runs[0].bold = True
        p_estudante = doc.add_paragraph(f"Destinatário: {nome_estudante}")
                    ...
                    if img_bytes:
                        try:
                            ...
                            if isinstance(data, bytes):
                                ...
                                r = p.add_run()
                                r.add_picture(BytesIO(data), width=Inches(4.5))
                                doc.add_paragraph()
                        except Exception:
                            pass
                elif parte.strip():
